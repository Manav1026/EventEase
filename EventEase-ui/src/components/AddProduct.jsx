...
//   const [mediaType, setMediaType] = useState("");
      ...
      complete: async (results) => {
        for (const row of results.data) {
            const checkSkuExists = async () => {
                const res = await fetch(`http://localhost:3000/api/vendor/check-sku/${row.sku}`);
            ...
            if (row.discountedPrice && parseFloat(row.discountedPrice) > parseFloat(row.price)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const checkSkuExists = async () => {
            const res = await fetch(`http://localhost:3000/api/vendor/check-sku/${sku}`);
            const data = await res.json();
            return data.exists;
          };
          const skuExists = await checkSkuExists();
